...
# - Team A = Cyan/Blue players (left side score)
# - Team B = Red players (right side score)
# - Yellow/Gold outline = MVP/top fragger, assigned to team with 4 players (5 cyan + 4 red → yellow is red, 5 red + 4 cyan → yellow is cyan)
#
...
def _row_team_from_patches(patches: List[np.ndarray], row_idx: int = None):
    ...
    Team A (Cyan/Blue) = Higher blue score
    Team B (Red) = Higher red score
    
    Yellow/Gold players are detected separately and assigned based on team counts.
    
    Returns: ("A", is_gold) or ("B", is_gold)
    - "A" = Cyan/Blue team
    - "B" = Red team
    - is_gold = True if player has yellow/gold outline (MVP/top fragger)
    """
    b_list, r_list, golds = [], [], []
    for p in patches:
...
class OCRScanner(commands.Cog):
    ...
    async def scan(self, interaction: discord.Interaction, image: discord.Attachment):
        ...
        try:
            ...
            # ============ AGENT DETECTION: ROBOFLOW ============
            # Save temp image for Roboflow agent detection
            ...
            detected_agents = []
            # Get map from parsed data and translate if Chinese
            raw_map = parsed.get('map', 'Unknown')
            detected_map = translate_map_name(raw_map) if raw_map else 'Unknown'
            agent_detection_success = False
            detection_method = "Roboflow"

            try:
                print("\n" + "="*70)
                print("Starting Roboflow agent detection...")
                print("="*70)
                
                # Initialize Roboflow detector
                if not self.roboflow_detector:
                    from services.roboflow_agent_detector import RoboflowAgentDetector
                    self.roboflow_detector = RoboflowAgentDetector()
                    print("Roboflow detector initialized")
                
                # Detect agents
                detections = self.roboflow_detector.detect_with_visualization(
                    str(temp_image_path),
                    confidence=50  # 50% confidence threshold
                )
                
                print(f"Roboflow detected {len(detections)} agents")
                
                # Sort by position and extract agent names
                detections_sorted = sorted(detections, key=lambda d: d.get('position', 999))
                detected_agents = [d.get('agent', 'Unknown') for d in detections_sorted[:10]]
                
                # Pad to 10 agents
                while len(detected_agents) < 10:
                    detected_agents.append('Unknown')
                
                agent_detection_success = len([a for a in detected_agents if a != 'Unknown']) >= 5
                
                print(f"Agent detection success: {agent_detection_success}")
            ...
            # Apply uniqueness constraint - DISABLED
            # DISABLED: Allow duplicate agents per team
            ...
            # First pass: detect cyan (blue) and red players
            for idx, p in enumerate(players[:10]):
                ...
                team_color = "Cyan" if team == "A" else "Red"
                print(f"  Row {idx+1} ({ign}): Blue={blue_total:.3f}, Red={red_total:.3f} → {team_color} {'⭐GOLD/YELLOW' if has_gold else ''}")
                
                if has_gold:
                    # Store gold/yellow players for second pass
            ...
            # Second pass: assign gold/yellow players to the team with 4 players
            # Logic: If 5 red + 4 cyan detected → yellow is cyan
            #        If 5 cyan + 4 red detected → yellow is red
            print("🌟 === GOLD/YELLOW PLAYER ASSIGNMENT ===")
            for idx, gold_p in gold_players:
                assigned_team = None
                ign = gold_p.get('ign', 'Unknown')
                
                # Primary logic: assign to the team with 4 players (the incomplete team)
                if len(team_a) == 4 and len(team_b) == 5:
                    team_a.append(gold_p)
                    assigned_team = "A (Cyan)"
                    print(f"  ✅ {ign}: Assigned to Team A (4 cyan + 5 red → yellow is cyan)")
                elif len(team_b) == 4 and len(team_a) == 5:
                    team_b.append(gold_p)
                    assigned_team = "B (Red)"
                    print(f"  ✅ {ign}: Assigned to Team B (5 cyan + 4 red → yellow is red)")
                elif len(team_a) == 4 and len(team_b) < 5:
                    # Team A needs the gold player
                    team_a.append(gold_p)
                    assigned_team = "A (Cyan)"
                    print(f"  ✅ {ign}: Assigned to Team A (Team A has 4 players)")
                elif len(team_b) == 4 and len(team_a) < 5:
                    # Team B needs the gold player
                    team_b.append(gold_p)
                    assigned_team = "B (Red)"
                    print(f"  ✅ {ign}: Assigned to Team B (Team B has 4 players)")
                else:
                    # Fallback: use color detection for gold player
                    patches = _sample_color_patches(pil, idx)
                    team, _ = _row_team_from_patches(patches)
                    
                    # Check underlying team color (red vs cyan)
                    b_list, r_list = [], []
                    for patch in patches:
                        b, r, _ = _score_patch(patch)
                        b_list.append(b)
                    ...
                    if team == "A":
                        team_a.append(gold_p)
                        assigned_team = "A (Cyan)"
                        print(f"  🎨 {ign}: Color detection → Team A (Blue={blue_total:.3f}, Red={red_total:.3f})")
                    else:
                        team_b.append(gold_p)
                        assigned_team = "B (Red)"
                        print(f"  🎨 {ign}: Color detection → Team B (Blue={blue_total:.3f}, Red={red_total:.3f})")
            
            print(f"After gold assignment: Team A={len(team_a)}, Team B={len(team_b)}\n")
            ...
            # DISABLED: team_a_agents = {p.get('agent', 'Unknown'): p for p in team_a if p.get('agent', 'Unknown') != 'Unknown'}
            # DISABLED: team_b_agents = {p.get('agent', 'Unknown'): p for p in team_b if p.get('agent', 'Unknown') != 'Unknown'}
            
            # Find agents that appear on both teams
            # DISABLED: cross_duplicates = set(team_a_agents.keys()) & set(team_b_agents.keys())
            
            # DISABLED: if cross_duplicates:
                # DISABLED: print(f"  ⚠️ Found {len(cross_duplicates)} agents on BOTH teams: {cross_duplicates}")
                
                # DISABLED: for agent in cross_duplicates:
                    # DISABLED: player_a = team_a_agents[agent]
                    # DISABLED: player_b = team_b_agents[agent]
                    
                    # DISABLED: conf_a = player_a.get('agent_confidence', 0)
                    # DISABLED: conf_b = player_b.get('agent_confidence', 0)
                    
                    # Keep the one with higher confidence, mark other as Unknown
                    # DISABLED: if conf_a >= conf_b:
                        # DISABLED: print(f"    Keeping {agent} in Team A (conf: {conf_a:.2f}) - removing from Team B")
                        # DISABLED: player_b['agent'] = 'Unknown'
                        # DISABLED: player_b['agent_source'] = 'Cross_Duplicate_Removed'
                    # DISABLED: else:
                        # DISABLED: print(f"    Keeping {agent} in Team B (conf: {conf_b:.2f}) - removing from Team A")
                        # DISABLED: player_a['agent'] = 'Unknown'
                        # DISABLED: player_a['agent_source'] = 'Cross_Duplicate_Removed'
            # DISABLED: else:
                # DISABLED: print("✅ No cross-team duplicates - all agents unique!")
            ...
            # ============ SCORE ASSIGNMENT ============
            # FIXED LOGIC: Left score (top) = Cyan (Team A), Right score (bottom) = Red (Team B)
            score = parsed.get("score")
            
            if isinstance(score, dict) and ("top" in score or "bottom" in score):
                score_left = score.get("top") or 0   # Left score = Cyan = Team A
                score_right = score.get("bottom") or 0  # Right score = Red = Team B
                
                # Direct assignment: Left = Team A (Cyan), Right = Team B (Red)
                score_ab = {"A": score_left, "B": score_right}
                print(f"\n📊 Score Assignment:")
                print(f"   Team A (Cyan/Left): {score_left}")
                print(f"   Team B (Red/Right): {score_right}")
            else:
                score_ab = score
                print(f"⚠️ Score format unexpected: {score}")
            ...
            # Skip database for testing - just use empty list
            try:
                all_players = await db.get_all_players()
            except Exception as e:
                print(f"[INFO] Skipping database (testing mode): {e}")
                all_players = []
                ...
                if isinstance(score_ab, dict) and score_ab.get("A") is not None and score_ab.get("B") is not None:
                    ...
                    for idx, scanned_player in enumerate(players[:10]):
                        ...
                        if ign_lower in ign_to_player:
                            ...
                            # Track agent usage\n                            await self.save_agent_usage(\n                                discord_id,\n                                player.get("agent", "Unknown"),\n                                int(kills or 0),\n                                int(deaths or 0),\n                                int(assists or 0),\n                                is_mvp=is_mvp_player\n                            )\n\n                            updated_count += 1
            ...
            def format_player_line(p, idx):
                ...
                if is_registered and ign_lower in player_stats_cache:
                    ...
                    try:
                        ...
                        # Calculate OLD points (before this match)
                        registration_cog = self.bot.get_cog("Registration")
                        if registration_cog:
                            old_points = int(registration_cog.calculate_player_score(player_data))
                            
                            # Calculate NEW points (after this match)
                            k_val = int(k) if str(k).isdigit() else 0
                            d_val = int(d) if str(d).isdigit() else 0
                            a_val = int(a) if str(a).isdigit() else 0
                            
                            # Simulate adding this match's stats
                            new_stats = player_data.copy()
                            new_stats['kills'] = player_data.get('kills', 0) + k_val
                            new_stats['deaths'] = player_data.get('deaths', 0) + d_val
                            new_stats['assists'] = player_data.get('assists', 0) + a_val
                            new_stats['matches_played'] = player_data.get('matches_played', 0) + 1
                            # Assume win for now (can be refined based on team result)
                            new_stats['wins'] = player_data.get('wins', 0) + 1
                            new_stats['losses'] = player_data.get('losses', 0)
                            
                            new_points = int(registration_cog.calculate_player_score(new_stats))
            ...
            try:
                ...
                if match_result.get('duplicate'):
                    # Create duplicate warning embed
                    dup_emb = discord.Embed(
                        title="⚠️ Duplicate Match Detected",
                        description=f"This match appears to be identical to **Match #{match_result['existing_match_id']}**.\n\n**Do you want to submit it anyway?**",
                        color=discord.Color.orange()
                    )
                    
                    # Create confirmation view with Submit/Decline buttons
                    class DuplicateConfirmView(discord.ui.View):
                        def __init__(self, match_data, original_emb):
                            super().__init__(timeout=300)  # 5 minute timeout
                            self.match_data = match_data
                            self.original_emb = original_emb
                            self.value = None
                        
                        @discord.ui.button(label="Submit Anyway", style=discord.ButtonStyle.green, emoji="✅")
                        async def submit_button(self, interaction: discord.Interaction, button: discord.ui.Button):
                            await interaction.response.defer()
                            
                            # Force save the match
                            try:
                                result = await db.force_save_match_results(self.match_data)
                                
                                # Update the embed
                                self.original_emb.color = discord.Color.green()
                                self.original_emb.title = "✅ Match Saved (Duplicate Override)"
                                self.original_emb.description = None
                                self.original_emb.add_field(
                                    name="Match ID", 
                                    value=f"#{result['match_id']}", 
                                    inline=False
                                )
                                ...
                                await interaction.message.edit(embed=self.original_emb, view=None)
                                await interaction.followup.send("✅ Match has been saved!", ephemeral=True)
                            except Exception as e:
                                await interaction.followup.send(f"❌ Error saving match: {e}", ephemeral=True)
                        ...
                        async def decline_button(self, interaction: discord.Interaction, button: discord.ui.Button):
                            ...
                            # Update the embed
                            self.original_emb.color = discord.Color.red()
                            self.original_emb.title = "❌ Duplicate Match Declined"
                            self.original_emb.description = "Match scan cancelled."
                            
                            self.value = False
                            self.stop()
                            
                            await interaction.message.edit(embed=self.original_emb, view=None)
                            await interaction.followup.send("Match scan cancelled.", ephemeral=True)
                    
                    # Send with confirmation buttons
                    view = DuplicateConfirmView(match_result['match_data'], dup_emb)
                    await interaction.followup.send(embed=dup_emb, view=view)
    ...
    async def edit_agent(self, interaction: discord.Interaction, player_ign: str, agent: str):
        ...
        try:
            ...
            # Save match data to PostgreSQL database
            # Save match data to PostgreSQL database
            try:
                print("[DB] Preparing to save match data...")
                print(f"[DB] Available locals: {list(locals().keys())}")
                
                # Prepare match data - using actual variable names from the scan
                match_data = {
                    'map': 'Unknown',  # Map detection not implemented yet
                    'team_a': 'Team A',  # Default team names
                    'team_b': 'Team B',
                    'team_a_rounds': 0,  # These need to be extracted from OCR
                    'team_b_rounds': 0,
                    'winner': ''
                }
                
                print(f"[DB] Match data prepared: {match_data}")
                print(f"[DB] Saving match...")
                
                # Note: team_a_players and team_b_players should exist from agent detection
                # Save to database
                match_id = await self.save_match_to_database(match_data, team_a_players, team_b_players)
                print(f"[DB] Match saved! ID: {match_id}")
            except NameError as e:
                print(f"[DB ERROR] Variable not found: {e}")
                print(f"[DB ERROR] This means team player lists weren't created properly")
            except Exception as e:
